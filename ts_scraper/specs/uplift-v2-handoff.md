...
| Group header | Expanded vs collapsed | Each `<h3>` has a `<button>` toggle (mobile only — desktop never collapses). Persists per-group expansion in `localStorage` under key `job_filters:filter-group:{id}`. |
...
**Why**: the role page is the conversion surface. Today it's an unstyled column; it should be the most "job_filters" page on the site. The editorial treatment puts the comp band — the single most-clicked spec on any posting — in a pullquote, and uses the fact card to surface every `Role` field including ones currently hidden ("first seen", "last seen", department).
...
i. **Document title change**: continue to `document.title = "{role.title} at {role.company} · job_filters"` in `onMount` after role load — required for share-link unfurls.
