...
A row of pills below the mode panel persists `q` snapshots to `localStorage` under `job_filters:v1:saved-searches` ([site/src/lib/storage.ts](../site/src/lib/storage.ts)).
...
`localStorage["job_filters:v1:saved"]` / `:applied` / `:ignored` already store `Job.id[]` ([site/src/lib/storage.ts](../site/src/lib/storage.ts)). The filter UI exposes a **single-select toggle** that narrows the result set to one of those lists:
...
Three lists, namespaced under `job_filters:v1:`:

- `job_filters:v1:saved` — `string[]` of `Job.id`
- `job_filters:v1:applied` — `string[]` of `Job.id` with optional timestamp
- `job_filters:v1:ignored` — `string[]` of `Job.id`
...
Migration on schema bump: read the old version, transform, write the new version, retain a backup under `job_filters:vN-prev:*` for one release cycle.
...
- Group headers are also collapse toggles on mobile only — per-group expansion persists in `localStorage` under `job_filters:filter-group:{id}` ([site/src/lib/group-storage.ts](../site/src/lib/group-storage.ts)).
