...
- **User-Agent identifies the project**: `job_filters/<version> (+<contactUrl>)` — never spoofed to look like a browser. Per-ATS UA rotation is reserved for cases where an ATS hard-blocks shared-tooling UAs; documented in the per-ATS section if it applies.
