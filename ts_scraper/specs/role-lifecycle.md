...
`localStorage["job_filters:v1:{saved,applied,ignored}"]` continues to store `Job.id[]`. The carry-forward semantics specifically protect saved-job lookups: a role going stale does not change its `Job.id`, so a saved role's lookup still resolves until the role drops past TTL.
