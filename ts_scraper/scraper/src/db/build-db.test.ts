...
import { ATS_IDS, type Job, jobId, type ScrapeOutput, type Tenant } from "@job_filters/shared";
...
describe("buildDb carry-forward integration", () => {
  ...
  it("stamps is_stale=0 on fresh rows and is_stale=1 on carried rows", () => {
    tmpDir = mkdtempSync(join(tmpdir(), "job_filters-carry-"));
  ...
  it("drops rows once last_seen_at is older than the TTL", () => {
    tmpDir = mkdtempSync(join(tmpdir(), "job_filters-ttl-"));
