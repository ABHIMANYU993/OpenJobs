...
function tmpDir(): string {
  return mkdtempSync(join(tmpdir(), "job_filters-cli-"));
}
...
describe("runReportCommand", () => {
  ...
  it("renders a report and writes to --output", async () => {
    ...
    expect(md).toContain("job_filters run report — abc1234");
  ...
  it("writes the report to stdout when --output is omitted", async () => {
    ...
    expect(captured).toContain("job_filters run report");
  });
  ...
  it("dispatches harvest via main() too", async () => {
    ...
    try {
      ...
      const code = await main([
        ...
        "--skip-probe",
        "--user-agent",
        "job_filters-test/0.0.0 (+https://example.invalid)",
        "--contact-url",
        "https://example.invalid/contact",
      ]);
      expect(code).toBe(0);
