 ...
 *
 * Bun workspace invocations (`bun run --filter @job_filters/scraper harvest`)
...
function usage(): void {
  console.error(`
job_filters-scrape — schema ${SCHEMA_VERSION}

Usage:
  job_filters-scrape <command> [options]
...
export async function runHarvestCommand(argv: ReadonlyArray<string>): Promise<number> {
  ...
  if (args.userAgent !== undefined) {
    ...
    userAgent = `job_filters/${SCHEMA_VERSION} (+${args.contactUrl})`;
  } else {
    console.error(
