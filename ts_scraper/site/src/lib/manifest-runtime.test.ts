...
describe("fetchManifest", () => {
  it("fetches the manifest from {basePath}/data/manifest.json", async () => {
    ...
    const m = await fetchManifest("/job_filters", fakeFetch);
    expect(receivedUrl).toBe("/job_filters/data/manifest.json");
    expect(m.total_rows).toBe(100);
  });
  ...
  it("throws on non-2xx", async () => {
    ...
    await expect(fetchManifest("/job_filters", fakeFetch)).rejects.toThrow(/503/);
  });
});
